...
public class JobBuilderUTest extends EasyMockSupport {
    ...
    public void prepareEverythingWithWrongSource() {
        ...
        try {
            createJsonNode(true, null);
            expect(remoteClient.getConnectionUrl()).andReturn("");
            expect(remoteClient.getJsonNodeFromUrl(anyObject(String.class))).andThrow(
                new RemoteException("Source not found"));
    ...
    public void prepareEverythingWithOtherJenkinsException() throws IOException, RemoteException {
        ...
        expect(remoteClient.getConnectionUrl()).andReturn("");
        expect(remoteClient.getJsonNodeFromUrl(anyObject(String.class))).andThrow(
            new RemoteException("Another Error has occurred"));
    ...
    private void createJsonNode(boolean building, String result) throws IOException {
        jsNode = new ObjectMapper()
            .readTree("{\"actions\":[{\"causes\":[{\"shortDescription\":\"Build wurde durch eine SCM-Änderung ausgelöst.\"}]},{},{\"failCount\":0,\"skipCount\":7,\"totalCount\":2633,\"urlName\":\"testReport\"}],\"building\":"
                + building
                + ",\"description\":null,\"fullDisplayName\":\"A1OpenNet » A1ON-java-build-trunk #5710\",\"id\":\"2015-08-03_16-12-39\",\"result\":"
                + result + ",\"timestamp\":1438611159844}");
    }
}

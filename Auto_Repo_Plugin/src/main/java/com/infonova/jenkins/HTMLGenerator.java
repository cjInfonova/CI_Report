...
public class HTMLGenerator implements OutputGenerator {
    ...
    public void buildTable(List<Job> list, BufferedWriter bwr, String name, List<Failure> failList, String colname)
        ...
        bwr.write("<table class=tg><tr><th class=tgheader style=background-color:rgb(" + color.getRed() + ","
            + color.getGreen() + "," + color.getBlue() + ") colspan=3>" + name + "</th></tr>");
        bwr.newLine();
        bwr.write("<tr style=background-color:rgb(" + red + "," + green + "," + blue
            + ")><td class=tg>Step</td><td class=tg>Ergebnis</td><td class=tg>Zuletzt gr&uuml;n</td></tr>");
        bwr.newLine();
        ...
        for (Job r : list) {
            ...
            bwr.write("<tr style=background-color:rgb(" + red2 + "," + green2 + "," + blue2
                + ")><td class=tg><a href=https://ci.infonova.at/job/A1OpenNet/job/" + r.getJobName() + ">"
                + r.getJobName() + "</a></td></a><td class=tg>" + r.getResult() + "<sup>" + hoch + "</sup>"
                + "</td><td class=tg>" + r.getLastStableDate() + "</td></tr>");
            bwr.newLine();
            hoch = "";
            bool = false;
        }
    ...
    public void staticPostCode(BufferedWriter bwr, String sonar, String codecove) throws IOException {
    ...
    public void buildFailureTable(List<Job> jobList, BufferedWriter bwr, String systemName, List<Failure> failList,

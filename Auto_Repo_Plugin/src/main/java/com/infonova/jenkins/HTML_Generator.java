 ...
 */
    public class HTML_Generator {
        ...
        protected void buildTable(List<Job> list, BufferedWriter bwr, String name,List<Failure> failList) throws IOException {
            bwr.write("<table class=tg><tr><th class=tg-" + name + "h colspan=" + 3
                    + ">" + name + "</th></tr>");
            bwr.newLine();
            bwr.write("<tr><td class=tg-" + name + "th>Step</td><td class=tg-" + name + "th>Ergebnis</td><td class=tg-" + name + "th>Zuletzt gr&uuml;n</td></tr>");
            bwr.newLine();
            ...
            for (Job r : list) {
                ...
                bwr.write("<tr><td class=tg-" + name + "td><a href=https://ci.infonova.at/job/A1OpenNet/job/" + r.getJobName()
                        + ">" + r.getJobName() + "</a></td></a><td class=tg-" + name + "td>" + r.getResult() + "<sup>" + hoch + "</sup>"
                        + "</td><td class=tg-" + name + "td>" + r.getLastStableDate() + "</td></tr>");
                bwr.newLine();
                hoch="";
                bool=false;
            }
                ...
                for (Failure f : failList) {
                    ...
                    if(f.getJobName().equals(before))
                    {
                        ...
                        if (bool && !f.getFailure().equals("NoFailure") && f.getJobName().equals(j.getJobName())) {
                            bwr.write("<tr style=font-size:16px ><td></td><td>" + f.getClassname() + "</b></td><td>" + f.getFailure().substring(0, custlength)+"</td><td>"+f.getAge()+"</td></tr>");
                            bwr.newLine();
                        }
                    }
                    else{
                        ...
                        if (f.getJobName().equals(j.getJobName()) && !f.getFailure().equals("NoFailure")) {
                            bwr.write("<tr style=font-size:16px><td><b>" + i + "</td><td>" + f.getClassname() + " </b></td><td> " + f.getFailure().substring(0, custlength) + "</td><td>"+f.getAge()+"</td></tr>");
                            bwr.newLine();
                            bool = true;
                        }
                    }
